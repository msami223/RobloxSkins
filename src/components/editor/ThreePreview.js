...
export default function ThreePreview() {
  ...
  const createBasePodium = (scene, logoText = 'custom use') => {
    ...
    podiumMesh.position.set(0, -0.2, 0)
  ...
  useEffect(() => {
    ...
    if (!rendererRef.current) {
      ...
      scene.background = new THREE.Color('rgba(248, 250, 252, 0)') // Light background
    ...
    modelRef.current.traverse((child) => {
      if (child.isMesh) {
        ...
        if (name.includes('ghost')) {
          ...
          mat.opacity = 1
          if (mat.color) mat.color.setHex(0xffffff) // White skin tone
  ...
  return (
    ...
    >
      <div ref={containerRef} style={{ flex: 1, width: '100%' }}></div>
