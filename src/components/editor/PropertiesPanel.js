...
const FONT_FILES = [
  'ACOME Regular.ttf',
  'Acidente-Regular.otf',
  'Action Comics.ttf',
  'Adventure.otf',
  'Akira Expanded Demo.otf',
  'Alice_in_Wonderland_3.ttf',
  'Alien-Encounters-Bold.ttf',
  'Amari_Font_14-100-Regular.otf',
  'ArtegraSansSC-Regular.otf',
  'Autolova.ttf',
  'Basilisk-Free.otf',
  'BrinkyMono-Basic.ttf',
  'Bruks-Display.otf',
  ...
  'ROMANUS.otf',
  'Rakkas-Regular.ttf',
  'Rellathage Rough.ttf',
  'Saiyan-Sans.ttf',
  'SevenElevenSerif.otf',
  'Seychelles.ttf',
  'Shotgunwedding-W0OE.otf',
  'Skrapbook.ttf',
  'Southam Demo.otf',
  'Splinter-Regular.otf',
  'Starborn.ttf',
  'StrayCat.otf',
  'StretchPro.otf',
  'TERMINAT.TTF',
  'Thorne Shaded.ttf',
  'TrajanusBricks.ttf',
  'WhiteRabbit-47pD.ttf',
  'abnes.otf',
  'achilles3out.ttf',
  'agencyfb-bold.ttf',
  'andres.ttf',
  'apexnew-medium-opentype.otf',
  'coolvetica rg.ttf',
  'fake receipt.ttf',
  'moscowmetro.otf',
  'moscowmetrocolor.otf',
  'paladins.ttf',
  'techno_hideo.ttf',
]
...
export default function PropertiesPanel() {
  ...
  // Inject Custom Fonts & Google Fonts
  React.useEffect(() => {
    // 1. Local Custom Fonts
    ...
    // 2. Google Fonts
    const googleFontId = 'google-fonts-link'
    if (!document.getElementById(googleFontId)) {
      const link = document.createElement('link')
      link.id = googleFontId
      link.rel = 'stylesheet'
      // Construct URL: names with spaces need '+'
  ...
  // Combine lists for UI
  const ALL_FONTS = [
    ...GOOGLE_FONTS,
    ...FONT_FILES.map((f) => f.replace(/\.(ttf|otf)$/i, '')),
  ].sort()
  ...
  const handleDrop = (e, targetIndex, targetType) => {
    ...
    const canvas =
      targetType === 'shirt' ? fabricRefShirt.current : fabricRefPants.current
    if (!canvas) return
    ...
    return displayLayers.map((layer, i) => (
      <div
        ...
        style={{
          ...layerItemStyle,
          borderColor:
            activeLayerId === layer.id ? 'var(--primary)' : 'var(--border)',
        }}
        onClick={() => {
          const canvas =
            layer.canvasTarget === 'shirt'
      ...
      >
        ...
        <div
          style={{
            ...
            borderRadius: '3px',
            backgroundColor:
              layer.canvasTarget === 'shirt' ? '#dbeafe' : '#fce7f3',
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          ...
          <button
            onClick={(e) => {
              e.stopPropagation()
              updateLayerOrder(layer.id, 'down')
            }}
            style={layerBtnStyle}
            title="Move down"
          >
      ...
      case 'upload':
        return (
          <div className="panel-content">
            ...
            >
              <label className="upload-btn" style={btnStyle}>
                ...
                <input
                  ...
                  accept="image/*"
                  hidden
                  onChange={(e) => handleUpload(e, fabricRefShirt)}
                />
              </label>
              <label className="upload-btn" style={btnStyle}>
                ...
                <input
                  ...
                  accept="image/*"
                  hidden
                  onChange={(e) => handleUpload(e, fabricRefPants)}
                />
              </label>
            </div>
            ...
            >
              ...
              >
                ...
                <input
                  ...
                  accept="image/*"
                  hidden
                  onChange={(e) => handleUVUpload(e, 'shirt')}
                />
              </label>
              <label
                className="upload-btn"
              ...
              >
                ...
                <input
                  ...
                  accept="image/*"
                  hidden
                  onChange={(e) => handleUVUpload(e, 'pants')}
                />
              </label>
            </div>
            ...
            >
              <strong>Quick Upload:</strong> Centers image on canvas.
              <br />
        ...
        return (
          <div className="panel-content">
            ...
            >
              ...
              >
                ...
                <button
                  onClick={() => {
                    const newPriority =
                      torsoPriority === 'shirt' ? 'pants' : 'shirt'
              ...
              <div style={{ fontSize: '0.85rem', color: '#0c4a6e' }}>
                Using:{' '}
                <strong>{torsoPriority === 'shirt' ? 'Shirt' : 'Pants'}</strong>{' '}
                texture
              </div>
            </div>
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <div style={layerGroupStyle}>{renderUnifiedLayerList()}</div>
            </div>
          </div>
        )
      case 'text':
        return (
          <div className="panel-content">
            <h2 style={headerStyle}>Add Text</h2>

            {/* Target Selector */}
            <div style={{ marginBottom: '20px' }}>
              <label style={subHeaderStyle}>Target Canvas</label>
              <div style={{ display: 'flex', gap: '10px' }}>
                <button
                  onClick={() => updateTextOptions({ target: 'shirt' })}
                  style={{
                    ...toolBtnStyle,
                    backgroundColor:
                ...
                >
                  <i className="fa-solid fa-shirt"></i> Shirt
                </button>
                <button
                  onClick={() => updateTextOptions({ target: 'pants' })}
                  style={{
                    ...toolBtnStyle,
                    backgroundColor:
                      textOptions.target === 'pants'
                        ? 'rgba(99, 102, 241, 0.1)'
                        : 'var(--bg-workspace)',
                    borderColor:
                      textOptions.target === 'pants'
                ...
                >
                  <i className="fa-solid fa-socks"></i> Pants
                </button>
              </div>
            </div>
            ...
            {/* Text Input */}
            <div style={{ marginBottom: '20px' }}>
              <label style={subHeaderStyle}>Content</label>
              <textarea
                value={textOptions.text}
                onChange={(e) => updateTextOptions({ text: e.target.value })}
            ...
            <div style={{ marginBottom: '20px', position: 'relative' }}>
              <label style={subHeaderStyle}>Font</label>

              <div
                onClick={() => {
                  // Toggle dropdown - use a ref or just simple state?
                  // We need local state for this dropdown.
                  // Since we can't easily add state to this functional component without fully rewriting,
                  // let's use a simple detail: we are inside PropertiesPanel which IS a component.
                  // We need to add `isFontOpen` state at top.
                  // Wait, I can't add state here. I need to update the component definition above first.
                  // BUT, I can implement it right here if I assume the state exists.
                  // Let's assume I will add `const [isFontOpen, setIsFontOpen] = useState(false)` at the top level.
              ...
              >
                ...
                <i
                  className={`fa-solid fa-chevron-${isFontOpen ? 'up' : 'down'}`}
              ...
              {isFontOpen && (
                ...
                >
                  <div
                    onClick={() => {
                      updateTextOptions({ fontFamily: 'Arial' })
                      setIsFontOpen(false)
                    }}
                    style={{
                      padding: '10px 12px',
                    ...
                    onMouseEnter={(e) => {
                      if (textOptions.fontFamily !== 'Arial')
                  ...
                  {ALL_FONTS.map((name) => {
                    return (
                      <div
                        key={name}
                        onClick={() => {
                          updateTextOptions({ fontFamily: name })
                          setIsFontOpen(false)
                        }}
                        style={{
                          padding: '10px 12px',
                        ...
                        onMouseEnter={(e) => {
                          if (textOptions.fontFamily !== name)
            ...
            <div style={{ display: 'flex', gap: '15px', marginBottom: '20px' }}>
              <div style={{ flex: 1 }}>
                <label style={subHeaderStyle}>
                  Size: {textOptions.fontSize}px
                </label>
                <input
                  type="range"
                  min="10"
                  ...
                  onChange={(e) =>
                    updateTextOptions({ fontSize: parseInt(e.target.value) })
                  }
                  style={{ width: '100%', cursor: 'pointer' }}
                />
              </div>
              <div>
                <label style={subHeaderStyle}>Color</label>
                <input
                  type="color"
                  value={textOptions.color}
                  onChange={(e) => updateTextOptions({ color: e.target.value })}
                  style={{
                    width: '40px',
                    height: '40px',
                    padding: '0',
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button
                onClick={() =>
                  updateTextOptions({ isBold: !textOptions.isBold })
                }
                style={{
                  ...toolBtnStyle,
                  backgroundColor: textOptions.isBold
                    ? 'rgba(99, 102, 241, 0.1)'
                    : 'var(--bg-workspace)',
                  borderColor: textOptions.isBold
                    ? 'var(--primary)'
                    : 'var(--border)',
                  fontWeight: 'bold',
                }}
              ...
              <button
                onClick={() =>
                  updateTextOptions({ isItalic: !textOptions.isItalic })
                }
                style={{
                  ...toolBtnStyle,
                  backgroundColor: textOptions.isItalic
                    ? 'rgba(99, 102, 241, 0.1)'
                    : 'var(--bg-workspace)',
                  borderColor: textOptions.isItalic
                    ? 'var(--primary)'
                    : 'var(--border)',
                  fontStyle: 'italic',
                }}
            ...
            >
              Text is centered automatically. Switch target to place on Shirt or
              Pants.
            </p>
          </div>
        )
