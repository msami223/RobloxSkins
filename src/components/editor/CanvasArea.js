...
export default function CanvasArea() {
  ...
  useEffect(() => {
    ...
    const loadAndDrawUVFromModel = async (
      fabricCanvas,
      wireframeRef,
      meshName,
      uvTrianglesRef,
          ...
          (gltf) => {
            ...
            model.traverse((child) => {
              if (child.isMesh) {
                ...
                if (
                  (meshName === 'shirt' && name.includes('top')) ||
            ...
            if (triangles.length > 0) {
              ...
              console.log(
                `Loaded ${triangles.length} UV triangles for ${meshName} with real-time clipping`
              )
            } else {
  ...
  // Generate star stamp
  const createStarStamp = useCallback(
    (size, color, opacity) => {
  ...
  const createHeartStamp = useCallback(
    (size, color, opacity) => {
      const canvas = document.createElement('canvas')
      // Add padding for shadow to prevent clipping
      const padding = brushSoftness > 1 ? brushSoftness * 3 : 0
      ...
      ctx.bezierCurveTo(
        totalSize / 2,
        size * 0.6 + centerOffset,
        centerOffset,
        size * 0.5 + centerOffset,
        centerOffset,
        topCurveHeight + centerOffset
      )
      ctx.bezierCurveTo(
        centerOffset,
        centerOffset,
  ...
  const createFlowerStamp = useCallback(
    (size, color, opacity) => {
      const canvas = document.createElement('canvas')
      // Add padding for shadow to prevent clipping
      const padding = brushSoftness > 1 ? brushSoftness * 3 : 0
  ...
  const createLightningStamp = useCallback(
    (size, color, opacity) => {
      const canvas = document.createElement('canvas')
      // Add padding for shadow to prevent clipping
      const padding = brushSoftness > 1 ? brushSoftness * 3 : 0
  ...
  // 1. Sync Sidebar -> Canvas Active Object
  useEffect(() => {
    // Only run if Text tab is active and we have textOptions changes
    if (activeTab !== 'text') return

    const targetCanvas =
      textOptions.target === 'shirt'
        ? fabricRefShirt.current
        : fabricRefPants.current
    if (!targetCanvas) return

    // Check if text object already exists
    let textObj = targetCanvas
      .getObjects()
      .find((obj) => obj.id === 'custom-text-tool')

    // If target changed, remove from other canvas
    const otherCanvas =
      textOptions.target === 'shirt'
        ? fabricRefPants.current
        : fabricRefShirt.current
    if (otherCanvas) {
      const otherText = otherCanvas
        .getObjects()
        .find((obj) => obj.id === 'custom-text-tool')
      if (otherText) {
        otherCanvas.remove(otherText)
        otherCanvas.renderAll()
      }
    }

    if (!textObj) {
      // Create new text object (Interactive!)
      textObj = new fabric.IText(textOptions.text, {
        id: 'custom-text-tool',
        left: CANVAS_WIDTH / 2,
        top: CANVAS_HEIGHT / 2,
        originX: 'center',
        originY: 'center',
        selectable: true, // Interactive
        evented: true,
        hasControls: true,
        hasBorders: true,
      ...
      // Update existing, ONLY if values differ (to prevent loop)
      const needsUpdate =
        textObj.text !== textOptions.text ||
        textObj.fontFamily !== textOptions.fontFamily ||
        textObj.fontSize !== textOptions.fontSize ||
        textObj.fill !== textOptions.color ||
        (textObj.fontWeight === 'bold') !== textOptions.isBold ||
        (textObj.fontStyle === 'italic') !== textOptions.isItalic
    ...
    debouncedTextureUpdate()
    syncLayers()
  }, [activeTab, textOptions, debouncedTextureUpdate, syncLayers])

  // 2. Handle Canvas Mode Switching & Interaction Events
  useEffect(() => {
    const shirtCanvas = fabricRefShirt.current
    const pantsCanvas = fabricRefPants.current
    if (!shirtCanvas || !pantsCanvas) return

    const canvases = [shirtCanvas, pantsCanvas]

    // Setup Event Listeners for Sync & 3D Update
    const handleObjectModified = () => {
      debouncedTextureUpdate()
      syncLayers()
    }

    // When object is selected/modified on canvas -> Update Sidebar?
    // Note: To avoid infinite loops, we might only want to update sidebar on SELECTION, not every keystroke if it causes re-render.
    // Ideally, sidebar reflects the ACTIVE object.
    ...
    canvases.forEach((canvas) => {
      ...
      if (activeTab === 'text') {
        ...
        // Enable interaction for Text objects
        canvas.getObjects().forEach((obj) => {
          if (obj.id === 'custom-text-tool') {
            obj.set({ selectable: true, evented: true })
          } else {
            // Lock other drawings so we only select text? Or allow all?
            // User asked "Ensure that addition of text object does not interfere with drawing mode"
            // Usually in Text mode, we only want to manipulate text.
        ...
        canvas.on('object:moving', () => debouncedTextureUpdate()) // Real-time 3D update
        canvas.on('object:scaling', () => debouncedTextureUpdate())
        canvas.on('object:rotating', () => debouncedTextureUpdate())

        // Sync Selection -> Sidebar
        const handleSelection = (e) => {
          const selected = e.selected?.[0]
          if (selected && selected.id === 'custom-text-tool') {
            // Update sidebar state to match selected text
            // We need to access updateTextOptions... but it's not in the dependency array or scope?
            // It's available from useEditor().
            // However, updating state inside an event listener might be tricky with closures if we don't have fresh ref.
            // But updateTextOptions is stable from Context usually.

            // Wait, we need to import updateTextOptions or pass it in.
            ...
            updateTextOptions({
              ...
              color: selected.fill,
              isBold: selected.fontWeight === 'bold',
              isItalic: selected.fontStyle === 'italic',
              target: canvas === shirtCanvas ? 'shirt' : 'pants',
            })
          }
        }
        ...
        // Disable interaction for ALL objects while drawing
        canvas.getObjects().forEach((obj) => {
          obj.set({ selectable: false, evented: false })
        })
      } else {
        // Upload/Layers tab - maybe allow selection?
        canvas.isDrawingMode = false
        canvas.selection = true
        canvas.getObjects().forEach((obj) => {
          obj.set({ selectable: true, evented: true })
  ...
  }, [activeTab, debouncedTextureUpdate, syncLayers, updateTextOptions])

  // Generate spiral stamp
  const createSpiralStamp = useCallback(
    (size, color, opacity) => {
      const canvas = document.createElement('canvas')
      // Add padding for shadow to prevent clipping
      const padding = brushSoftness > 1 ? brushSoftness * 3 : 0
  ...
  const createSquareStamp = useCallback(
    (size, color, opacity) => {
      const canvas = document.createElement('canvas')
      // Add padding for shadow to prevent clipping
      const padding = brushSoftness > 1 ? brushSoftness * 3 : 0
  ...
  const createTriangleStamp = useCallback(
    (size, color, opacity) => {
      const canvas = document.createElement('canvas')
      // Add padding for shadow to prevent clipping
      const padding = brushSoftness > 1 ? brushSoftness * 3 : 0
  ...
  const createDiamondStamp = useCallback(
    (size, color, opacity) => {
      const canvas = document.createElement('canvas')
      // Add padding for shadow to prevent clipping
      const padding = brushSoftness > 1 ? brushSoftness * 3 : 0
  ...
  // Generate text stamp
  const createTextStamp = useCallback(
    (size, color, opacity) => {
  ...
  useEffect(() => {
    ...
    const useCustomDrawing =
      (brushSpacing > 20 || isCustomStampBrush) && activeTab === 'draw'

    if (!useCustomDrawing) return

    const canvases = [
      {
        canvas: fabricRefShirt.current,
        el: document.querySelector('#canvas-shirt'),
      },
      {
        canvas: fabricRefPants.current,
        el: document.querySelector('#canvas-pants'),
      },
    ]
  ...
  useEffect(() => {
    ...
    if (fabricRefShirt.current)
      fabricRefShirt.current.isDrawingMode = useBuiltInDrawing
    if (fabricRefPants.current)
      fabricRefPants.current.isDrawingMode = useBuiltInDrawing
