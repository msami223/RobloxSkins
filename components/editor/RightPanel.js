...
const DEFAULT_COLORS = [
  '#FF0000', '#000000', '#FFFFFF', // Red (default), Black, White
  '#EF4444', '#F97316', '#FACC15', '#22C55E', '#06B6D4', '#3B82F6', // Row 1
  '#6366F1', '#A855F7', '#EC4899', '#F43F5E', // Row 2
  '#78716C', '#1E293B', // Grays
]
...
export default function RightPanel() {
  ...
  const handleDeleteLayer = () => {
    ...
    let activeObject = shirtCanvas?.getActiveObject() || pantsCanvas?.getActiveObject()
    let targetCanvas = shirtCanvas?.getActiveObject() ? shirtCanvas : (pantsCanvas?.getActiveObject() ? pantsCanvas : null)
    ...
    if (!activeObject && activeLayerId) {
      ...
      if (layer && layer.object) {
        ...
        targetCanvas = layer.canvasTarget === 'shirt' ? shirtCanvas : pantsCanvas
      }
    }
    
    // If still no object, delete the most recent object from either canvas
    if (!activeObject) {
      const shirtObjs = shirtCanvas?.getObjects().filter(o => !o.excludeFromExport) || []
      const pantsObjs = pantsCanvas?.getObjects().filter(o => !o.excludeFromExport) || []
        ...
        <div className="panel-content">
            ...
            <div style={{ 
              ...
              {DEFAULT_COLORS.map((color, i) => (
                <div
                  ...
                  style={{
                    ...
                    cursor: 'pointer',
                    border: brushColor.toUpperCase() === color.toUpperCase() ? '2px solid var(--primary)' : '2px solid var(--border)',
                    boxShadow: brushColor.toUpperCase() === color.toUpperCase() ? '0 0 0 2px rgba(99, 102, 241, 0.3)' : 'none',
              ...
              <label style={{
                ...
                <input 
                  type="color" 
                  value={brushColor}
                  onChange={(e) => updateBrush({ color: e.target.value, eraser: false })}
                  style={{ 
                    position: 'absolute',
                    width: '100%',
                    height: '100%',
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <input 
              ...
              max="100" 
              value={brushOpacity} 
              onChange={(e) => updateBrush({ opacity: parseInt(e.target.value) })}
              style={rangeStyle}
            />
          </div>
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <input 
              ...
              max="100" 
              value={brushSoftness} 
              onChange={(e) => updateBrush({ softness: parseInt(e.target.value) })}
              style={rangeStyle}
            />
          </div>
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <input 
              ...
              max="100" 
              value={brushSpacing} 
              onChange={(e) => updateBrush({ spacing: parseInt(e.target.value) })}
              style={rangeStyle}
            />
          ...
          <div style={{ display: 'flex', gap: '10px', marginBottom: '16px' }}>
            <button 
              ...
              style={{ 
                ...toolBtnStyle, 
                flex: 1,
                backgroundColor: !isEraser ? 'var(--primary)' : 'var(--bg-workspace)', 
    ...
    if (activeLayer) {
      return (
        <aside style={panelStyle}>
          ...
          <div style={{padding:'10px', background:'var(--bg-workspace)', borderRadius:'8px', border:'1px solid var(--border)'}}>
            <div style={{fontSize:'14px', fontWeight:'500', marginBottom:'4px', color: 'var(--text-main)'}}>{activeLayer.type || activeLayer.name}</div>
            <div style={{fontSize:'12px', color:'var(--text-muted)'}}>Selected</div>
          </div>
        </aside>
      )
    }
